...
for (n in c(4, 8, 10, 20, 40)) {
  for (i in 1:50) {
    ...
    # Sampling
    set.seed(i)
    sample1 <- sample.int(48, n/2) 
    sample2 <- sample.int(47, n/2) + 48
    ...
    ## CAMERA
    result_table_camera <- camera_voom_m1(samp_exprs, samp_meta, dds)
...
fwrite(summary_table_fora, './M1/summary_table_fora_m1.tsv', sep = '\t') 
fwrite(summary_table_camera, './M1/summary_table_camera_m1.tsv', sep = '\t') 
fwrite(summary_table_fgsea, './M1/summary_table_fgsea_m1.tsv', sep = '\t')
fwrite(summary_table_SE, './M1/summary_table_fgsea_SE_m1.tsv', sep = '\t')
